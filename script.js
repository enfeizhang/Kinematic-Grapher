...
document.querySelector("#axes-btn").addEventListener("click", () => {
 const rawaxessequence = `G90
G0 X10 Y190 ${zdown} F3000
G0 X190 Y190 F3000
G0 X185 Y195 F1000
G0 X185 Y185 F1000
G0 X190 Y190 F1000
...
document.querySelector("#dotline-btn").addEventListener("click", () => {
 const dotlinesequence = `G91
G0 X160 Z2 F4000
G0 Z-2 F1000
G0 Y-10 F1000
G0 Y-10 Z2 F1000
...
G0 Z-2`;
  ...
  if (fab.isPrinting) {
    fab.commands = dotlinesequence
      .split("\n")
      .map(line => checkGcode(line.trim()));

    fab.print();
    pushMessage("sent dotted line sequence", "blue");
  } else {
    pushMessage("initialize printer first", "red");
  }
});
...
G0 X10 Y190 ${zup} F4000`

    if (fab.isPrinting) {
    fab.commands = originreturnsequence
      .split("\n")
      .map(line => checkGcode(line.trim()));

    fab.print();
    pushMessage("sent origin return sequence", "blue");
  } else {
    pushMessage("initialize printer first", "red");
  }
...
const returnoriginandpendown = `G90
G0 X10 Y190 Z${zup} F4000
G0 Z${zdown} F1000`

  if (fab.isPrinting) {
    fab.commands = returnoriginandpendown
      .split("\n")
      .map(line => checkGcode(line.trim()));

    fab.print();
    pushMessage("sent origin and pen down sequence", "blue");
  } else {
    pushMessage("initialize printer first", "red");
  }

const userspeedbox = document.querySelector("#speed-input");
const __vx = Number(userspeedbox.value);
const y_dist = -(50 * 160)/ __vx
const f_speed = 60 * (__vx**2 + (50**2))** 0.5

const drawgraphsequence = `G90
G0 X10 Y190 Z${zdown} F8000
G91
G0 X180 Y${y_dist} F${f_speed}
G0 Z3 F300`

 if (fab.isPrinting) {
    fab.commands = drawgraphsequence
      .split("\n")
      .map(line => checkGcode(line.trim()));

    fab.print();
    pushMessage("sent graphing sequence", "blue");
  } else {
    pushMessage("initialize printer first", "red");
  }
addSpeedOl(document.querySelector("#speedsol"), __vx)
userspeedbox.value = ""
})
